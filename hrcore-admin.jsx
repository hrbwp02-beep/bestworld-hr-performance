...
/* ================= รับพนักงานจากระบบสรรหา (STEP 19) ================= */
function HRCRecruitIntake() {
  const [f, setF] = useA1({
    candidate_id: "", application_id: "", name: "", dept: (HRC.departments[0] || {}).id || "",
    position_id: "", email: "", phone: "", birth_date: "", hire_date: new Date().toISOString().slice(0, 10),
    citizen_id: "", employment_type: "prob", supervisor_id: "",
  });
  const [busy, setBusy] = useA1(false);
  const [result, setResult] = useA1(null);
  const set = (k, v) => setF((p) => ({ ...p, [k]: v }));
  const canCreate = HRC.can("employee.create");
  const fromRecruit = (HRC.employees || []).filter((e) => e.source === "recruit");

  const submit = async () => {
    if (!f.candidate_id.trim() || !f.name.trim()) { setResult({ ok: false, error: "ต้องระบุรหัสผู้สมัครและชื่อ" }); return; }
    setBusy(true); setResult(null);
    const { data, error } = await window.sb.rpc("hr_create_employee_from_candidate", {
      p_candidate_id: f.candidate_id.trim(), p_name: f.name.trim(), p_dept: f.dept,
      p_position: f.position_id ? HRC.posName(f.position_id) : null,
      p_application_id: f.application_id.trim() || null,
      p_email: f.email.trim() || null, p_phone: f.phone.trim() || null,
      p_birth_date: f.birth_date || null, p_hire_date: f.hire_date || null,
      p_employment_type: f.employment_type, p_citizen_id: f.citizen_id.replace(/\D/g, "") || null,
      p_position_id: f.position_id || null, p_supervisor_id: f.supervisor_id || null,
    });
    setBusy(false);
    if (error) { setResult({ ok: false, error: error.message }); return; }
    setResult(data);
    if (data && data.ok) { await HRC.load(); toast("สร้างพนักงาน " + data.employee_code + " แล้ว", "check"); }
  };

  return (
    <div className="grid">
      <Crumb items={[{ label: "HR Core" }, { label: "รับพนักงานจากสรรหา" }]} />
      <div className="page-head">
        <div><h1>รับพนักงานจากระบบสรรหา</h1><p>สร้างพนักงานจากผู้สมัครที่ผ่านการอนุมัติ · ตรวจซ้ำอัตโนมัติ</p></div>
      </div>

      <div style={{ background: "var(--accent-soft)", color: "var(--accent-700)", borderRadius: 12, padding: "12px 16px", fontSize: 13, lineHeight: 1.75 }}>
        ระบบสรรหาส่งข้อมูลผู้สมัครเข้ามาที่นี่ได้ 2 ทาง — กรอกในฟอร์มนี้ หรือเรียกฟังก์ชัน
        <span className="mono"> hr_create_employee_from_candidate </span> ผ่าน API
        โดยระบบจะ <b>ตรวจผู้สมัครซ้ำ → ตรวจพนักงานซ้ำ → ออกรหัสพนักงาน → สร้างพนักงาน → เชื่อมกลับผู้สมัคร → บันทึก Audit</b> ให้อัตโนมัติ
      </div>

      {!canCreate && <div style={{ background: "var(--red-soft)", color: "#be123c", borderRadius: 11, padding: "11px 15px", fontSize: 13 }}>
        <Icon name="lock" size={15} /> ต้องมีสิทธิ์ <b>employee.create</b> จึงจะสร้างพนักงานได้</div>}

      <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit,minmax(330px,1fr))" }}>
        <Card>
          <CardHead title="ข้อมูลผู้สมัครที่ผ่านอนุมัติ" sub="กรอกข้อมูลเพื่อสร้างเป็นพนักงาน" />
          <div className="card-pad" style={{ display: "flex", flexDirection: "column", gap: 13 }}>
            <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div className="field"><label>รหัสผู้สมัคร *</label><input className="input mono" value={f.candidate_id} onChange={(e) => set("candidate_id", e.target.value)} placeholder="candidate_id" /></div>
              <div className="field"><label>รหัสใบสมัคร</label><input className="input mono" value={f.application_id} onChange={(e) => set("application_id", e.target.value)} placeholder="application_id" /></div>
            </div>
            <div className="field"><label>ชื่อ-นามสกุล *</label><input className="input" value={f.name} onChange={(e) => set("name", e.target.value)} /></div>
            <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div className="field"><label>หน่วยงาน *</label>
                <select className="select" value={f.dept} onChange={(e) => { set("dept", e.target.value); set("position_id", ""); }}>
                  {(HRC.departments || []).map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}
                </select></div>
              <div className="field"><label>ตำแหน่ง</label>
                <select className="select" value={f.position_id} onChange={(e) => set("position_id", e.target.value)}>
                  <option value="">— ไม่ระบุ —</option>
                  {(HRC.positions || []).filter((p) => !f.dept || p.department_id === f.dept).map((p) => <option key={p.id} value={p.id}>{p.position_name}</option>)}
                </select></div>
            </div>
            <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div className="field"><label>วันเริ่มงาน</label><input className="input" type="date" value={f.hire_date} onChange={(e) => set("hire_date", e.target.value)} /></div>
              <div className="field"><label>ประเภทการจ้าง</label>
                <select className="select" value={f.employment_type} onChange={(e) => set("employment_type", e.target.value)}>
                  {(HRC.employmentTypes || []).filter((t) => t.is_active).map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}
                </select></div>
            </div>
            <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div className="field"><label>วันเกิด</label><input className="input" type="date" value={f.birth_date} onChange={(e) => set("birth_date", e.target.value)} /></div>
              <div className="field"><label>เลขบัตรประชาชน</label><input className="input mono" value={f.citizen_id} onChange={(e) => set("citizen_id", e.target.value)} /></div>
            </div>
            <div className="grid" style={{ gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <div className="field"><label>อีเมล</label><input className="input" value={f.email} onChange={(e) => set("email", e.target.value)} /></div>
              <div className="field"><label>เบอร์โทร</label><input className="input" value={f.phone} onChange={(e) => set("phone", e.target.value)} /></div>
            </div>
            <div className="field"><label>ผู้บังคับบัญชา</label>
              <select className="select" value={f.supervisor_id} onChange={(e) => set("supervisor_id", e.target.value)}>
                <option value="">— ไม่ระบุ —</option>
            ...
            {result && (
              <div style={{ borderRadius: 10, padding: "11px 14px", fontSize: 13,
                ...
                {result.ok
                  ? <>สร้างพนักงานสำเร็จ · รหัส <b>{result.employee_code}</b></>
                  : <>ไม่สำเร็จ: {result.error}{result.employee_id ? " (พนักงาน " + result.employee_id + ")" : ""}</>}
              </div>
            )}
            <button className="btn btn-pri" onClick={submit} disabled={busy || !canCreate}>
              <Icon name="plus" size={16} />{busy ? "กำลังสร้าง…" : "สร้างเป็นพนักงาน"}
            </button>
          </div>
        </Card>

        <Card>
          <CardHead title="พนักงานที่มาจากระบบสรรหา" sub={fromRecruit.length + " คน"} />
          <div style={{ padding: "8px 14px", maxHeight: 460, overflowY: "auto" }}>
            {fromRecruit.length === 0
              ? <EmptyState icon="users" title="ยังไม่มีพนักงานจากระบบสรรหา" sub="เมื่อสร้างพนักงานจากผู้สมัครแล้วจะแสดงที่นี่" />
              : fromRecruit.map((e) => (
                <div key={e.id} className="between" style={{ padding: "10px 4px", borderBottom: "1px solid var(--border-2)", gap: 10 }}>
                  <div className="row" style={{ gap: 9, minWidth: 0 }}>
                    <Avatar name={e._fullName} initials={(e._fullName || "?").trim()[0]} color={e.color || "#0d9488"} size={28} />
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>{e._fullName}</div>
                      <div className="mono muted" style={{ fontSize: 11 }}>{e.employee_code} · ผู้สมัคร {e.source_candidate_id || "—"}</div>
                    </div>
                  </div>
                  <Badge cls={HRC.statusMeta(e._status).cls} dot>{HRC.statusMeta(e._status).label}</Badge>
                </div>
              ))}
          </div>
        </Card>
...
Object.assign(window, { HRCUsers, HRCRoles, HRCAudit, HRCRecruitIntake });
